defmodule Day02.CorruptionChecksum do
  ...
  def part1(input \\ @input) do
    calculate_checksum(input, &max_min/1)
  end

  defp calculate_checksum(input, extractor) do
    input
    |> String.split("\n")
    |> Enum.filter(&(&1 != ""))
  ...
  defp max_min(row) do
    ...
    |> then(fn row_of_nums -> Enum.max(row_of_nums) - Enum.min(row_of_nums) end)
  end

  def part2(input \\ @input) do
    calculate_checksum(input, &divisibiles/1)
  end

  def divisibiles(row) do
    row_of_nums = Enum.map(row, &String.to_integer/1)

    for dividend <- row_of_nums, divisor <- row_of_nums, reduce: 0 do
      acc ->
        if dividend != divisor and rem(dividend, divisor) == 0,
          do: acc + div(dividend, divisor),
          else: acc
    end
  end
end
